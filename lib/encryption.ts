...
if (!process.env.ENCRYPTION_KEY || process.env.ENCRYPTION_KEY.length < 32) {
    console.warn(
        "Warning: ENCRYPTION_KEY is not set or is too short. Using fallback key for development only.",
    );
}

if (!process.env.ENCRYPTION_IV || process.env.ENCRYPTION_IV.length < 16) {
    console.warn(
        "Warning: ENCRYPTION_IV is not set or is too short. Using fallback IV for development only.",
    );
}
